...
import contentfulSpaceExportFile from "@/lib/ctf-exported-space.json";
...
export async function cleanSpace(client: any) {
  ...
  const totalContentTypes = contentTypes.total;
...
async function addLocales(client: any, locales: any[]) {
  try {
    // Fetch the current locales in the space to avoid adding duplicates

    const currentLocales = await client.locale.getMany({});

    const existingLocaleCodes = currentLocales.items.map(
      (locale: any) => locale.code
    );
    console.log("%%currentLocales", existingLocaleCodes);
    // Loop through the locales in your exported JSON file and add any missing ones
    for (const locale of locales) {
      if (!existingLocaleCodes.includes(locale?.code)) {
        ...
        console.log(`Locale ${locale?.name} already exists.`);
      }
    }

    console.log("Locales added successfully.");
  } catch (error) {
    console.error("Error adding locales:", error);
    throw error;
  }
}
...
export async function seedTheSpace({ managementToken, spaceId, envId }: any) {
  try {
    ...
    // Add locales from the exported JSON file
    const locales = contentfulSpaceExportFile.locales || [];
    if (locales.length > 0) {
      await addLocales(client, locales);
    } else {
      console.log("No locales to add.");
    }
